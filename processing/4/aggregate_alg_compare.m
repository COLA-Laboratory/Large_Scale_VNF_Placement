...
src_folder = fullfile(root_path, 'processed', 'aggregate');

original = fullfile(src_folder, 'AlgorithmComparison');
reduced = fullfile(src_folder, 'AlgCmpReduced');

out_folder = fullfile(root_path, 'processed', 'algorithm_comparison');

topologies = ["DCell", "FatTree", "LeafSpine"];
sizes = ["500", "1000", "2000", "4000", "8000", "16000"];
algorithms = ["MOEAD", "IBEA", "NSGA-II", "PPLSD", "PPLS_Simple"];
...
for topology = topologies
    for model = algorithms
        ...
        times_out = [];
        for size = sizes
            original_folder = fullfile(original, topology, size, model);
            reduced_folder = fullfile(reduced, topology, size, model);
            
            hv_file = fullfile(reduced_folder, "aggregate.csv");
            times_file = fullfile(original_folder, "aggregate_times.csv");
