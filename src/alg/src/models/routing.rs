...
pub fn get_tables(
    ...
    mut routing_tables: Vec<RoutingTable>,
) -> Vec<RoutingTable> {
    let graph = &datacentre.graph;

    for server in bgn..end {
        let start = server;
        ecmp(start, graph, |info| {
            if use_compression {
                routing_tables[info.node].consider(server, info.prev, info.dist)
            } else {
                routing_tables[info.node].consider_uncompressed(server, info.prev, info.dist)
            }
        });
    }
...
impl RoutingTable {
    ...
    pub fn consider_uncompressed(&mut self, server: usize, direction: usize, distance: usize) {
...
mod tests {
    ...
    #[test]
    fn test_rt_consider_uncompressed() {
        let mut rt_a = RoutingTable::new();
        let mut rt_b = RoutingTable::new();

        // Single server
        rt_a.consider_uncompressed(0, 1, 1);
        rt_a.consider_uncompressed(0, 2, 1);
        rt_a.consider_uncompressed(0, 3, 1);
        rt_a.consider_uncompressed(0, 1, 4);
        rt_a.consider_uncompressed(0, 6, 5);

        // Different servers, mergeable
        rt_b.consider_uncompressed(0, 1, 1);
        rt_b.consider_uncompressed(1, 1, 2);
        rt_b.consider_uncompressed(2, 1, 3);
        rt_b.consider_uncompressed(3, 1, 2);
        ...
        // Different servers, mergeable but not merged
        assert_eq!(rt_b.ranges.len(), rt_b.values.len());
        assert_eq!(rt_b.ranges.len(), 4);

        assert_eq!(rt_b.ranges[0].start, 0);
        assert_eq!(rt_b.ranges[0].end, 1);
        assert_eq!(rt_b.values[0], vec![1]);

        assert_eq!(rt_b.ranges[1].start, 1);
        assert_eq!(rt_b.ranges[1].end, 2);
        assert_eq!(rt_b.values[1], vec![1]);

        assert_eq!(rt_b.ranges[2].start, 2);
        assert_eq!(rt_b.ranges[2].end, 3);
        assert_eq!(rt_b.values[2], vec![1]);

        assert_eq!(rt_b.ranges[3].start, 3);
        assert_eq!(rt_b.ranges[3].end, 4);
        assert_eq!(rt_b.values[3], vec![1]);
    }
}
