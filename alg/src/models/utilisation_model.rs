use super::iterative_queueing_model::IterativeQueueingModel;
...
use crate::models::{get_metrics, iterate_route, Server};
...
impl<'a> UtilisationModel<'a> {
        ...
        F: Fn(f64) -> f64,
    {
        ...
        let mut servers: Vec<Server> = vec![Server::new(); self.dc.num_servers];

        let mut sw_arr = vec![0.0; self.dc.num_components()];
...
mod tests {
    ...
    use crate::{
        load_topology,
        models::{datacentre::Topology, service::VNF},
        operators::mapping::find_routes,
    };

    #[test]
    pub fn test_evaluate() {
        let (dc, rt) = load_topology(&Topology::FatTree, 16).unwrap();

        let sw_sr = 1.0;
        let sw_ql = 10;

        let qm = IterativeQueueingModel::new(&dc, sw_sr, sw_ql, 500.0, 0, 1.0, 1.0);

        let util_model = UtilisationModel::new(&dc, qm, sw_sr, sw_ql);
